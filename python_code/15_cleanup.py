...
#should take array/list of times for array
...
def tint(y,  conf = 0.99):
            n = len(y)
            var = tdist.ppf((1 + conf)/2, n-1)
            std_error = (y.std() / np.sqrt(n))
            mean = np.mean(y)
            tont = tdist.interval(confidence = 0.99, df = n-1, loc = mean, scale =std_error)
            lower, upper = tont 
            return lower, upper 
...
def speedup(df, timer):
    ...
    for i,(name, group) in enumerate(grouped):
         val = group[timer].to_numpy()
         base_comp_time = val[0]
         speedup_run  = base_comp_time / val
         speedup_actual.extend(speedup_run)

    return speedup_actual
...
#6      25.3760          2.68414  12.2851    10.3191      8    2 
    ...
    #########################################################################################################################
    """ calls scatterplot with mads function, get metrics calls speedup, eff and wall clock time plots, other plot is for Piro wall time showing each run """
    ...
    #######################################################################################################################
    """ Calculate efficiency for timers and appends it to new df_final dataframe """
    ...
    """ melts original dataframe in order to create new dataframe dffff which is used for boxplot """
    ...
    sns.catplot(data= dfffff, x = 'Cores', y = 'Time', hue ='Run', col= 'Timer', kind= 'point', errorbar = scitest, capsize = 0.2, sharey= False, ax = ax )

    plt.savefig(f'000', dpi =300)
 """
    #convert seaborn plots, then plot one by one and overlay

    

    #plt.errorbar(data = dfffff, x ='Cores', y= 'Time', yerr=0.5, fmt='o', color='black', alpha = 0.5 )

    plt.savefig(f'Strip plot for all timers with errorbars', dpi = 300)


    ##################################################################################################################
    """ obtains mean of all timers to create bar plot that displays time consumption for each timer on each core """
    ...
    sns.catplot(data= gruuu, y = 'Time', hue = 'Timer', col= 'Cores', kind= 'bar',  errorbar= 'sd')

    plt.savefig(f'Bar plot timers', dpi = 300)


    ##############################################################################################################################
    """ Takes df_final, which has columns for timers and timer efficiency, and drops columns for timers """
    ...
    sns.catplot(data= eff_melt, x = 'Cores', y = 'Percentage',  col= 'Efficiency', kind= 'box')

    plt.savefig(f'Merged boxplot efficiency', dpi=300)
    print(eff_melt)
    ...
    # Get unique timers and cores
    unique_timers = dfffff['Timer'].unique()
    unique_cores = dfffff['Cores'].unique()

    # Reshape t_error to match the number of unique cores and timers
    #t_error = np.array(t_error).reshape(len(unique_cores), len(unique_timers), 2)

    # Define the offset for error bars
    offset = 2

    # Loop over each unique timer
    for i, timer in enumerate(unique_timers):
        # Filter dataframe for the current timer
        timer_df = dfffff[dfffff['Timer'] == timer]
        
        # Initialize lists to store mean and yerr values
        means = []
        yerrs = []
        
        # Loop over each unique core
        """  for j, core in enumerate(unique_cores):
            # Filter dataframe for the current core and timer
            core_timer_df = timer_df[timer_df['Cores'] == core]
            
            # Calculate mean value
            mean = core_timer_df['Time'].mean()
            
            # Find the corresponding yerr values
            #yerr = t_error[j, i]
            
            # Append mean and yerr values to lists
            means.append(mean)
            #yerrs.append(yerr) """
        
        base_time = timer_df['Time'].to_numpy()[0]
        ideal_times = []
            
        for i in range(len(cores)):
            
            ideal_times.append(base_time/(2**i))

        ideal_time_df = pd.DataFrame(
             {
                  'Cores': cores,
                  'Ideal Times': ideal_times 
        ...
        # Create the plot
        plt.figure()
        unique_xs= sorted(timer_df['Cores'].unique())
        sns.pointplot( data=timer_df, x=timer_df['Cores'], y=timer_df['Time'], errorbar = scitest, capsize = 0.3, errwidth= 0.75, join =False, markers=None )
        sns.boxplot(data=timer_df, x=timer_df['Cores'],    y=timer_df['Time'], showcaps= False, linewidth= 0.5, color= 'red', whis=(0,100), label = "CPU-Perl", showmeans =True, meanprops={"marker":"s","markerfacecolor":"white", "markeredgecolor":"blue", "markersize":4})
        sns.pointplot(data = ideal_time_df, x = 'Cores', y='Ideal Times', marker='o', label = "Ideal-Times", linestyle='--')
        plt.xlabel('Nodes')
    ...
    unique_effs = eff_melt['Efficiency'].unique()

    for i, timer in enumerate(unique_effs):
        # Filter dataframe for the current timer
        eff_df = eff_melt[eff_melt['Efficiency'] == timer]
                
        # Create the plot
        plt.figure()
        unique_xs= sorted(eff_df['Cores'].unique())
        sns.pointplot( data=eff_df, x=eff_df['Cores'], y=eff_df['Percentage'], errorbar = scitest, capsize = 0.3, errwidth= 0.75, join =False )
        sns.boxplot(data=eff_df, x=eff_df['Cores'],    y=eff_df['Percentage'], showcaps= False, linewidth= 0.5, color= 'red',whis=(0,100),showmeans =True, meanprops={"marker":"s","markerfacecolor":"white", "markeredgecolor":"blue"})
        plt.xlabel('Cores')
...
#speedup grouped by runs, x-axis will show core cases (4,8,16,32,64), y-axis will show speedup (1x,2x,4x,8x...)
#speedup  = base_comp time (time at 4 cores in 1 run)
        
    speedup_df = pd.DataFrame()
    
    for timer in timers:
         
         speed = speedup(df, timer)
    
         speedup_df[f"Speedup {timer}"] = speed
        
         df_sort = df.sort_values(by=['Run', 'Cores']) #########################

         df_fin = pd.concat([df_sort.reset_index(drop=True), speedup_df.reset_index(drop=True)], axis=1)

    df_sp = df_fin.drop(['Albany Piro', 'Total Fill Time', 'Precond', 'Total Lin'], axis =1 )
    speed_melt = df_sp.melt(id_vars=['Cores', 'Run'], var_name = 'Speedup', value_name = 'Scale')
    print(speed_melt)

    unique_speed = speed_melt['Speedup'].unique()

    for i, timer in enumerate(unique_speed):
        # Filter dataframe for the current timer
        sp_df = speed_melt[speed_melt['Speedup'] == timer]
                
        # Create the plot
        plt.figure()
        unique_xs= sorted(sp_df['Cores'].unique())
        sns.pointplot( data=sp_df, x=sp_df['Cores'], y=sp_df['Scale'], errorbar = scitest, capsize = 0.3, errwidth= 0.75, join =False )
        sns.boxplot(data=sp_df, x=sp_df['Cores'],    y=sp_df['Scale'], showcaps= False, linewidth= 0.5, color= 'red', whis=(0,100), showmeans =True, meanprops={"marker":"s","markerfacecolor":"white", "markeredgecolor":"blue"})
        plt.xlabel('Cores')
        plt.ylabel('Scale')
        plt.title(f'Box Plot with Mean Error Bars - Speedup: {timer}')
        plt.savefig(f'Speedup {timer} Boxplot with mean errror bars', dpi =300)
        #sns.catplot(data= eff_melt, x = 'Cores', y = 'Percentage',  col= 'Efficiency', kind= 'box')
