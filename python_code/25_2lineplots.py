...
unique_effs = eff_plot_2threads['Efficiency'].unique()
...
for i, timer in enumerate(unique_effs):
    ...
    sns.pointplot( data=eff_df_4threads, x=eff_df_4threads['Cores'], y=eff_df_4threads['Percentage'], errorbar = scitest, capsize = 0.3, color= 'blue', errwidth= 0.75, join =True , dodge = True)
    sns.boxplot(data=eff_df_4threads, x=eff_df_4threads['Cores'],    y=eff_df_4threads['Percentage'], showcaps= False, linewidth= 0.5, color= 'blue', label = '4threads', whis=(0,100), showmeans =True, meanprops={"marker":"s","markerfacecolor":"blue", "markeredgecolor":"blue"}, dodge= True)
    ...
    sns.boxplot(data=eff_df_2threads, x=eff_df_2threads['Cores'],    y=eff_df_2threads['Percentage'], showcaps= False, linewidth= 0.5, color= 'green', label = '2threads', whis=(0,100), showmeans =True, meanprops={"marker":"s","markerfacecolor":"green", "markeredgecolor":"green"}, dodge= True)
...
for core in cores:
    ...
    for timer in timers:
        ...
        log_mean, log_lower, log_upper= trimmed_ttest_bounds(np.log(core_group_4threads[timer]), np.log(core_group_2threads[timer]))
        mean_list.append({
            'Cores': core,
            'Timer': timer,
...
# %%
unique_timers = final_2threads_melt['Timer'].unique()
...
for i, timer in enumerate(unique_timers):
    ...
    plt.figure()
    #unique_sorted_4theads= sorted(timer_df_4threads['Cores'].unique())
    #4threads
    ...
    col_labels = ["Nodes", '2threads Speedup' , "99% CI: (LL, UL)" ]
